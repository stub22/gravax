...
trait VecDistrib extends VecDistribFragment {
	...
	def projectEstimCovars(keySyms : IndexedSeq[EntryKey], maxLevels : Int) : CovarMatrix
...
class VecDistribBinnedImpl(rootBN : BinNode) extends VecDistrib {
	...
	override def projectEstimCovars(keySyms: IndexedSeq[EntryKey], maxLevels: Int): CovarMatrix = {
		// We always get
		...
		val covApprox = if (maxLevels == 1) {
			...
			assert(sumOfWeights == one)

			type UnwtCov = BigDecimal
			type UnwtCovPair = (EntryKey, UnwtCov)
			type UnwtCovRow = IndexedSeq[UnwtCovPair]
			type WtCov = BigDecimal
			type WtCovPair = (EntryKey, WtCov)
			type WCEnt = (EntryKey, WtCov)
			val outStatsPerKey: IndexedSeq[StatEntry] = entryIdx.map(eidx => {
				...
				// TODO:  Could fold these values in fewer steps, without the map-copying
				val wtEntStatTups: Seq[(DBinWt, StatEntry, UnwtCovRow)] = binIdx.map(bidx => {
					...
					val covRow: UnwtCovRow = entryIdx.map(coentIdx => {
				...
				// JDK9+ has sqrt on BigDecimal. In Scala we may have to use Spire or access the Java object, or ...
